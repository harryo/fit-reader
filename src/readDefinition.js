...
function readDefinition(file, devDataFields) {
  ...
  const fieldDefinitions = nTimes(nFields).map(() =>
    getFieldDef(file.readBytes(3), messages)
  );
  ...
  if (devDataFields !== null) {
    ...
    const developerFieldDefinitions = nTimes(nDeveloperFields).map(() =>
      getDeveloperFieldDef(file.readBytes(3), devDataFields)
    );
